      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2">
            <span className="text-2xl font-semibold tracking-tighter text-squid-dark">
          ...
          <nav className="flex items-center justify-end">
            ...
            >
              ...
              <div 
                ref={activeBubbleRef}
                className="absolute top-1/2 -translate-y-1/2 h-10 bg-squid-pastel-blue/40 rounded-full transition-all duration-700 ease-in-out shadow-sm"
                style={{ opacity: 0, width: '80px', left: '0' }}
              ></div>
              ...
              {navLinks.map((link, index) => (
                <Link 
                  key={index} 
                  to={link.href}
                  className={`relative px-3 md:px-5 py-2 text-sm transition-all duration-300 rounded-full wavy-border z-10 ${
